...
`loadGitHubInbox(filters, read, now)` accepts an injectable async transport. Each call receives `{ kind: 'graphql', query: INBOX_GRAPHQL, variables: { search, cursor } }` and expects the unmodified GraphQL JSON envelope. `GitHubInbox({ read })` and `inboxQueryOptions(..., read)` use the same seam. The default adapter now calls `githubOperation` with a structured `github.api` GraphQL operation. The private backend supplies the selected account credential; forced host redaction applies to parsed JSON before serialization. Account/profile/connection generations fence each page and scope the cache/remount boundary. `INBOX_CONTEXT_CHANGED` prevents optional-team recovery from swallowing an identity change. See [account execution](account-execution-feasibility.md) for installation requirements.
...
The production GraphQL document was exercised read-only against GitHub: personal review search returned two PRs with expected field shapes, team-review-requested-user was accepted (zero matches for that account), and public cli/cli search returned 50 nodes with hasNextPage true. That earlier read-only probe verifies API shape and qualifier acceptance, not a positive team-membership fixture or an installed Desktop test. The integrated account backend replaces the old large-payload shell transport with structured, redacted JSON.
